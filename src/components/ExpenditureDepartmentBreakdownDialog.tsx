              ...
              <tr>
                  {columns.map((column) => {
                    ...
                    return (
                      <th
                        key={column}
                        className={`px-3 py-2 ${isFirstColumn ? 'text-left w-48 min-w-[12rem] max-w-[12rem] break-words' : 'text-left'} text-xs font-semibold text-foreground ${shouldHaveRightBorder(column) ? 'border-r-2 border-border' : ''}`}
                      >
                ...
                <tr key={row.id || index} className="hover:bg-muted/50 border-b border-border">
                  {columns.map((column) => {
                    ...
                    return (
                      <td
                        key={column}
                        className={`px-3 py-2 text-sm ${isFirstColumn ? 'w-48 min-w-[12rem] max-w-[12rem] break-words' : 'whitespace-nowrap'} ${
