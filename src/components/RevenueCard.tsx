  ...
  return (
    <Card className="bg-gradient-card border-border shadow-soft hover:shadow-medium transition-all duration-300" role="region" aria-label={title}>
      ...
      <CardContent>
        ...
        {change !== undefined && (
          <div className="flex items-center space-x-1 text-xs mt-1" aria-label={`Year over year change: ${isPositiveChange ? 'increase' : 'decrease'} of ${formatChange(change)}`}>
            {isPositiveChange && (
              <>
                <TrendingUp className="h-3 w-3 text-success" aria-hidden="true" />
                <span className="text-success" aria-hidden="true">{formatChange(change)}</span>
              </>
            )}
            {isNegativeChange && (
              <>
                <TrendingDown className="h-3 w-3 text-destructive" aria-hidden="true" />
                <span className="text-destructive" aria-hidden="true">{formatChange(change)}</span>
              </>
            )}
          </div>
        )}
